...
2 vCPU、4 GB 内存服务器默认仍使用一个构建 worker。自动堆上限会参考当前可用内存，按为运行中的站点和原生分配预留至少 1 GiB 余量来计算；连最低 768 MiB 堆上限都无法满足时，构建前退出。`NODE_MEMORY_LIMIT` 可显式覆盖传给 Node 的 V8 old-space 上限，但 Next.js 16.3.5 会移除静态生成 worker 的此项限制，因此它不能保证该 worker 或整个构建的内存上限。部署时仍应观察服务器峰值内存；空间不足时改用异机构建。
