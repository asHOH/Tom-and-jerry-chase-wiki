...
import { InvalidPublishOperationIdError, readPublishOperationId } from './publishOperation';
import {
...
import { isGameDataSubmitMode, type GameDataSubmitMode } from './submitMode';
import {
...
type SubmissionKind = 'ordinary' | 'relations';
...
function readActionItems(
  ...
  const items = rawItems.map((item) => {
    ...
    // Relations enforce this limit during strict preparation, after authorization.
    if (kind === 'ordinary' && entryCount > PUBLISH_LIMITS.topLevelEntries) {
...
async function notifyOrdinarySubmission(
  prepared: PreparedPublishRequest,
  results: readonly TrustedPublishResult[],
  ...
  if (pendingActionIds.length > 0) {
    try {
      ...
      console.error('Failed to publish pending game data action notifications:', notificationError);
    }
  }

  const recipientUserId = actorUserId;
...
/** Owns validation, authorization, acknowledgement, persistence, and HTTP response mapping. */
export async function handleGameDataSubmission(
  request: Request,
  kind: SubmissionKind
  ...
  const authorize = kind === 'relations' ? requirePermission : requirePermissionOrAnonymous;
  ...
  // Establish the actor before strict action decoding; scoped checks follow canonicalization.
  const guard = await authorize(permission, undefined, 'all', {
    request,
    blockAction: 'edit',
  });
  ...
  try {
    ...
    const resourceGuard = await authorize(permission, contexts, 'all', {
      request,
      blockAction: 'edit',
    });
    ...
    if (hasSupabaseAdminConfig()) {
      try {
        ...
        // Awareness is advisory; persistence still validates the approved candidate.
        console.warn(
          kind === 'relations'
            ? 'Pending relation action overlap check failed open:'
            : 'Pending game data action overlap check failed open:',
          pendingError
        );
      }
    }
    ...
    if (kind === 'ordinary') await notifyOrdinarySubmission(prepared, results, guard.userId);
