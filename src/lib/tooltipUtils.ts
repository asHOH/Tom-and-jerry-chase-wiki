...
const propertyTooltips = {
  cat: {
    ...
    detailed: {
      ...
      商店购物时间: '从局内商店购买道具所需的时间（多数猫为2.6s，全体鼠为4.1s）',
      性别: '角色的游戏内性别，会影响语音及塔拉相关技能判定',
      英文名: '角色的官方译名',
    },
  },
  mouse: {
...
const itemPositioningTagTooltips = {
  投掷类: '可被拾取，通过道具键瞄准和投掷的道具',
  手持类: '可被拾取，通过点击道具键直接使用，或用以触发部分交互的道具',
  物件类: '不可拾取，通过交互等方式进行使用的道具',
  食物类: '包括牛奶、蛋糕和饮料，是游戏内的常规恢复手段',
  流程类: '与游戏进程紧密关联的道具',
  特殊类: '特性和机制复杂、难以归类的道具',
  常规道具: '包括绝大多数道具',
  衍生道具: '包括只能由其他道具进一步衍生的特殊道具',
  地图道具: '包括只能在部分地图或模式中出现的道具(不包括场景组件)',
};

const entityPositioningTagTooltips = {
  /* types */
  投射类: '生成后沿抛物线、直线或类似轨迹运动，大都通过直接接触目标产生相应效果',
  触发类: '生成后不立刻产生效果，但在一定条件下（例如目标靠近或接触）会主动产生相应效果',
  物件类: '生成后持续时间较长，相对被动地产生自身效果',
  NPC: '具有与角色相近的外形或部分特性，部分情况下由AI操作',
  变身类: '在特定情况下，玩家可变身为此类衍生物',
  特殊类: '特性和机制复杂、难以归类的衍生物',
  /* tags */
  抛掷: '生成后沿抛物线运动',
  平射: '生成后沿直线或预设轨迹运动',
  追踪: '当有目标时，自身持续向目标位置运动',
  命中: '可触发与“命中”相关的知识卡/特技，例如缴械、精准投射等',
  触发: '在一定条件下（例如目标靠近或接触）会立即产生相应效果',
  延时: '在一定条件下（例如目标靠近或接触）会触发，但触发后仍需等待一段时间才会产生相应效果',
  功能: '持续存在，产生独特的效果',
  阻挡: '可以阻挡其它物体通行',
  指示: '通常无其它功能，仅用于指示',
  变形: '通过“为原角色提供变身buff”的方式实现的一类变身，此类变身期间受到的状态效果会反馈到“本体”上',
  变身: '通过“原角色消失，操作新角色”的方式实现的一类变身，此类变身的“本体”与“变身”实际上各自独立',
  彩蛋: '游戏内的彩蛋内容',
  星元: '“多元乱斗”中的一类特殊变身角色，被称作“星元变身”，需在局外单独解锁和升级',
  拾取: '可直接或在特定情况下被当作道具拾取，放置于道具栏',
  交互: '可被角色交互以触发相关效果',
  伤害: '对目标造成伤害',
  硬控: '对目标造成负面状态或效果',
  增益: '对目标施加增益',
...
const fixturePositioningTagTooltips = {
  平台类: '上方能够承载其它物体，通常无法移动',
  地面类: '上方能够承载其它物体，通常无法移动，且更难以被穿越',
  墙壁类: '会阻挡其它物体，通常无法移动，且更难以被穿越',
  组件类: '用于丰富地图玩法，具有各式各样的效果',
  流程类: '与游戏进程紧密关联',
  NPC: '具有与角色相近的外形或部分特性，由非玩家操作',
  可交互: '角色靠近/碰触该物体后，可通过交互键进行交互',
  通用组件: '在很多地图都会出现的组件',
  地图组件: '只在少数地图出现的组件，往往具有更加特殊的效果',
...
const rolePositioningTagTooltips = {
  ...
  视野参数: '决定角色的基础视野范围大小；注意：该属性与实际视野大小呈反比，即本属性越大，视野越小',
  重力参数: '决定角色受到的重力大小',
  初始道具: '角色初始的手持道具',
