...
const buffDefinitions: Record<string, BuffDefinition> = {
  ...
  隐身: {
    ...
    buffclass: '效果',
    description:
      '使自身角色模型对友方变为半透明，对敌方变为全透明，但从高处落地时产生的烟雾仍能被敌方看到。',
    detailedDescription:
      '使自身角色模型（包括昵称及Hp条等）对友方变为半透明，对敌方变为全透明，但从高处落地时产生的烟雾仍能被敌方看到。此外隐身状态还能免疫部分状态效果，例如斯派克的抓取等。\n另注：少部分{霸体}效果能免疫隐身。',
  ...
  隐藏模型: {
    ...
    buffclass: '效果',
    aliases: ['#隐藏.*模型', '%模型消失'],
    description: '使角色模型变为全透明，不再产生碰撞，但仍能被少部分范围类效果影响。',
    source:
  ...
  拾取: {
    ...
    aliases: ['#拾取'],
    description:
      '一次性效果。使角色拾取对应物品（包括道具/药水）并储存在对应键位上，但若对应位置已有其它物品则会掉落。',
    source:
