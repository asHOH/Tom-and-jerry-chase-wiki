...
const buffDefinitions: Record<string, BuffDefinition> = {
  ...
  提高交互速度: {
    ...
    description:
      '使所有或指定类型的交互速度按**百分比**提高，例如推奶酪、火箭救援、绑火箭速度等。其中“推速”特指推奶酪速度。',
  ...
  降低交互速度: {
    ...
    description:
      '使所有或指定类型的交互速度按**百分比**降低，例如推奶酪、火箭救援、绑火箭速度等。其中“推速”特指推奶酪速度。',
  ...
  受力: {
    ...
    aliases: ['#受.*力'],
    description:
      '使目标受力，改变水平和(或)竖直方向上的速度，但合速度不超过速度上限（即2000/秒）。',
    source:
  ...
  碰撞: {
    ...
    aliases: ['#碰撞', '#挤压', '#命中'],
    description:
      '投掷类道具/衍生物与其目标碰撞时产生命中效果；其它情况则使参与碰撞的双方受力，改变速度。',
    source: '有碰撞体积的物品（角色、道具、平台、地面、墙壁等）彼此接触时可能会发生碰撞。',
  ...
  不产生碰撞: {
    ...
    buffclass: '效果',
    aliases: ['#不.*产生.*碰撞'],
    description: '使角色不会与部分或全部物品发生{碰撞}。',
    source: '{隐藏模型}等状态附带不产生碰撞的效果。',
  ...
  保持惯性: {
    ...
    aliases: ['#保持惯性'],
    description:
      '使角色保持当前速度运动，不因按住方向键而改变移动速度，也不因松开方向键而停止运动。',
    source:
  ...
  无法移动: {
    ...
    description:
      '使角色无法进行移动操作，包括移动和跳跃。[飞行中的上下移动](此处仅包括天使汤姆和追风汤姆的飞行摇杆操作)不受影响。',
  ...
  解除指定状态: {
    ...
    detailedDescription:
      '若目标角色身上有指定类型的状态，则立即解除它。具备多个效果的状态在判定时，始终只会被判定为其中一个状态效果。',
    source:
  ...
  变身: {
    ...
    detailedDescription:
      '变身为指定类型的衍生物。变身分为两种：\n1.通过对自身施加变身状态进行变身：{变身饮料}、{头盔}、{大星星}、{小星星}、{线条猫}等。这类变身效果持续期间受到的伤害和效果均会实时反映到角色本体上；\n2.隐藏自身模型，并改为操作另一种指定类型的衍生物：{机器鼠(衍生物)}、{战车}、{乾坤袋(NPC)(衍生物)}、盔甲人等。这类变身效果的变身对象受到的伤害和效果不会对角色本体造成影响，但部分能对{模型消失}目标造成效果的技能仍能直接对本体造成影响。\n\n变身效果的优先级很高，通常不会因{霸体}、{虚弱}等效果而{免疫}。',
    source:
  ...
  击退: {
    ...
    source: '很多方式都能造成击退，例如{瑜伽球}、{魔音穿耳}等。',
    stack:
      '通常情况下各眩晕效果间互不影响，但当角色处于由**部分道具及极少部分技能**导致的眩晕时，免疫其它来自这些道具的眩晕。',
    detailedStack:
      '通常情况下各眩晕效果间互不影响，但当角色处于由{冰块}/{小鞭炮}/{鞭炮束}/{灰花瓶}/{蓝花瓶}/{狗骨头}/{2级剑客长枪}（蓄力超过2/3但未到最大值时）/{沙包拳头}（满怒游龙拳击退撞到道具）导致的眩晕时，免疫其它来自这些道具的眩晕。',
  ...
  连控保护: {
    ...
    source:
      '当**猫咪**在{常规眩晕}未结束时再次受到{常规眩晕}，则获得该状态，持续至猫咪身上的所有{常规眩晕}结束。例如猫在非眩晕状态下受到第1个眩晕效果，在该效果持续期间又受到了第2次眩晕，以及第3次和第4次眩晕，那么第3次和第4次眩晕的持续时间减少70%，直至此次眩晕效果解除，以此类推。',
    stack: '不可叠加。',
