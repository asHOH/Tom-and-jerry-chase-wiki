...
const buffDefinitions: Record<string, BuffDefinition> = {
  ...
  交互: {
    ...
    detailedStack:
      '具体效果受{提高交互速度}/{降低交互速度}的影响。通常来说，数值的固定值与百分比形式互相独立，各自加算，而后分别作为一个整体带入计算公式中。先结算百分比效果，后结算固定值效果。计算公式：基础速度×(1+百分比加速)×(1-百分比减速)+固定值加速-固定值减速，其中百分比加/减速单位为%。',
  ...
  提高交互速度: {
    ...
    stack:
      '通常来说，数值的固定值与百分比形式互相独立，各自加算，而后分别作为一个整体带入计算公式中。先结算百分比效果，后结算固定值效果。',
    detailedStack:
      '通常来说，数值的固定值与百分比形式互相独立，各自加算，而后分别作为一个整体带入计算公式中。先结算百分比效果，后结算固定值效果。计算公式：基础速度×(1+百分比加速)×(1-百分比减速)+固定值加速-固定值减速，其中百分比加/减速单位为%。',
  ...
  全局推速降低: {
    ...
    relate: ['降低交互速度'],
    description:
      '有2个同类效果：\n1.对局前3分钟内：使自身推奶酪速度降低20%。\n2.有队友被绑上火箭：使自身推奶酪速度降低10%。',
  ...
  全局推速提高: {
    ...
    relate: ['提高交互速度'],
    description:
      '有2个同类效果：\n1.对局3分钟后：使自身推奶酪速度提高20%。\n2.对局8分30秒后，且已有四块奶酪被推入时：使自身推奶酪速度提高50%。',
    source:
