...
const buffDefinitions: Record<string, BuffDefinition> = {
  ...
  提高跳跃高度: {
    ...
    source: '很多方式都能获得降低跳跃高度，例如{飞跃}、{鼓舞}等。',
  ...
  护盾: {
    ...
    description:
      '护盾能抵挡一次负面状态（例如{常规眩晕}等）或{虚弱}。部分护盾在抵挡指定类型的{负面状态}时不消耗层数。',
  ...
  无敌: {
    ...
    buffclass: '常规类',
    description:
      '状态持续期间能抵挡所有负面状态（例如{常规眩晕}等）或{虚弱}。该效果与护盾类似，但持续时间固定。',
    source:
  ...
  霸体: {
    ...
    detailedDescription:
      '{免疫}部分状态效果。[不同霸体效果免疫的范围不同](推测这是因为霸体的免疫实际上由“白名单”实现，在这个名单上的状态效果均会被免疫，而不同霸体的白名单差异很大)，会单独进行注明。大多数霸体均可免疫{常规眩晕}，此外各霸体常见的免疫项如下：{软控制}、{受伤}、{虚弱}、{隐身}。需注意即便是同类效果，霸体也有可能只能免疫其中的一至数种。',
  ...
  伤害转移: {
    bufftype: '正面效果',
    buffclass: '常规类',
    description: '将友方受到的伤害转移到自己身上，以使友方不受伤害。',
  ...
  控制转移: {
    bufftype: '正面效果',
    buffclass: '常规类',
    description: '将友方受到的负面控制效果（如{常规眩晕}）转移到自己身上，以使友方不受控制。',
    source: '{友情庇护}、{3级双重猫格}。',
  ...
  绑上火箭: {
    ...
    aliases: ['#被绑上火箭'],
    description:
      '无法移动、跳跃、使用技能和道具，同时以1/秒的基础速度减少自身所在火箭的倒计时；每隔一段时间可通过挣扎键增加挣扎进度条，进度条充满时根据挣扎类型触发+5秒/+10秒/[直接挣脱](现版本只能通过“幸运”或梦游杰瑞-1级“梦中乐园”触发)的效果。',
  ...
  禁用效果: {
    ...
    source:
      '很多方式都能造成禁用效果，例如{猛攻}、{红色卡牌}、{3级捕鼠专家}。此外{常规眩晕}、{虚弱}等状态效果也兼具部分禁用效果。',
    stack: '同类效果互不影响。',
  ...
  交互速度改变: {
    ...
    stack: '所有百分比加速/{减速}效果均为**乘算叠加**。',
    detailedStack:
      '所有百分比加速/{减速}效果均为**乘算叠加**。计算公式：基础速度×(1+加速)×……×(1-减速1)×……，其中加/减速单位为%。',
  ...
  全局推速降低: {
    ...
    description: '推奶酪速度降低20%。（注：本网站的角色基础推速已计入该状态的效果）。',
    source: '在经典模式的前3分钟内，所有老鼠均会受到该效果影响。',
  ...
  全局推速提高: {
    ...
    buffclass: '全局类',
    description:
      '推奶酪速度提高20%。（注：本网站的角色基础推速计入了{全局推速降低}的效果，因此实际速度应当为网站显示原数值的150%）',
    source: '在经典模式的3分钟之后，所有老鼠均会受到该效果影响。',
  ...
  鱼钩眩晕: {
    ...
    description:
      '与{眩晕}类似，但只会{击落老鼠}而不会{击落道具}，且由于是“正面效果”而不会被大部分{护盾}、{无敌}、{霸体}效果抵挡。',
    source: '只会由{尼宝的朋友}造成。',
