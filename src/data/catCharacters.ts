...
const catCharacterDefinitions: Record<string, CharacterDefinition | PartialCharacterDefinition> = {
  ...
  如玉: {
        ...
        skillLevels: [
          {
            level: 1,
            description:
              '[因其他原因虚弱时](即：不因本技能提供的“坚毅”导致虚弱时)改为进入“坚毅”状态：立即解除控制并回复Hp、且移速提高、免疫眩晕和虚弱，期间成功对任意敌方造成虚弱则提前解除该状态。“坚毅”状态正常结束时，如玉进入虚弱。',
            detailedDescription:
              '[因其他原因虚弱时](即：不因本技能提供的“坚毅”导致虚弱时)[改为进入12秒“坚毅”状态](仍会触发以虚弱为判定条件的效果，如鼠方的击倒敌方获得金钱效果及终结赏金，和知识卡-越挫越勇，暴怒，乘胜追击（失去层数）等)：立即解除控制并回复100Hp、且移速提高15%、免疫眩晕和虚弱，期间成功对任意敌方造成虚弱则提前解除该状态。“坚毅”状态正常结束时，如玉进入虚弱。',
          },
          {
            level: 2,
            description:
              '使如玉进入“坚毅”状态的敌方[在30秒内无法对如玉造成眩晕和虚弱](老鼠被抓起时提前解除该状态)。',
            detailedDescription:
              '使如玉进入“坚毅”状态的敌方[在30秒内无法对如玉造成眩晕和虚弱](老鼠被抓起时提前解除该状态)。',
          },
          {
            level: 3,
            description:
              '“坚毅”会回复更多Hp，且对敌方造成眩晕即可解除该状态。使如玉进入“坚毅”状态的敌方改为[在60秒内无法对如玉造成眩晕和虚弱](老鼠被抓起时提前解除该状态)。',
            detailedDescription:
              '“坚毅”改为回复200Hp，且对敌方造成眩晕即可解除该状态。使如玉进入“坚毅”状态的敌方改为[在60秒内无法对如玉造成眩晕和虚弱](老鼠被抓起时提前解除该状态)。',
          },
        ],
        description:
          '如玉开局手持道具替换为鞭炮束，从商店购买道具所需的时间较长。\n\n如玉无法释放常规爪刀，但拥有两种特殊爪刀，特殊爪刀[与技能类似](可在手中有老鼠时释放；可移动释放，但释放期间不能转向；可用道具键取消前后摇，但会进入CD；可打断需要其他技能键才能打断的技能；不会因缴械被封锁，但会被无法释放技能的效果封锁)，[无法触发常规爪刀效果](无法直接抓起被拍子控制的老鼠，无法为知识卡-乘胜追击叠加层数，无法破坏莱恩-蘸水笔创造的方块)，[无法受到大部分爪刀相关效果加成](无法受到知识卡-击晕、猛攻、蓄势一击、长爪；特技-全垒打，长爪一击，勇气爪刀影响)，但[CD与爪刀相同](可被影响爪刀CD的效果影响，如变身药水及特技-“我生气了！”。爪刀CD有一定概率不会被这些效果影响，疑似是bug，触发原因未知)。[两种特殊爪刀共用CD](共用爪刀CD，释放跳跃劈枪时会重置前刺回马枪的状态)。\n1.前刺回马枪：在地面时释放，分为2段：前刺枪对身前敌方造成少量伤害，命中非虚弱敌方时获得加速；回马枪对身后敌方造成少量伤害，击退对方一小段距离，若命中[被前刺枪命中过的非虚弱目标](指被同一轮前刺回马枪的第一段命中过的目标，包括初次命中时处于虚弱状态的目标)，则额外造成极高伤害和[短暂的强控制状态](该状态实际效果与眩晕类似，但角色头顶并未出现“眩晕”图标；动作与击退类似，但击退距离为0，且会替代回马枪原本的击退效果)。\n2.跳跃劈枪：在空中时释放，对身前大范围内的敌方造成伤害，命中非虚弱敌方时获得加速。',
        detailedDescription:
          '如玉开局手持道具替换为鞭炮束，从商店购买道具所需的时间较长（需要4秒）。\n\n如玉无法释放常规爪刀，但拥有两种特殊爪刀，特殊爪刀[与技能类似](可在手中有老鼠时释放；可移动释放，但释放期间不能转向；可用道具键取消前后摇，但会进入CD；可打断需要其他技能键才能打断的技能；不会因缴械被封锁，但会被无法释放技能的效果封锁)，[无法触发常规爪刀效果](无法直接抓起被拍子控制的老鼠，无法为知识卡-乘胜追击叠加层数，无法破坏莱恩-蘸水笔创造的方块)，[无法受到大部分爪刀相关效果加成](无法受到知识卡-击晕、猛攻、蓄势一击、长爪；特技-全垒打，长爪一击，勇气爪刀影响)，但[CD与爪刀相同](可被影响爪刀CD的效果影响，如变身药水及特技-“我生气了！”。爪刀CD有一定概率不会被这些效果影响，疑似是bug，触发原因未知)。[两种特殊爪刀共用CD](共用爪刀CD，释放跳跃劈枪时会重置前刺回马枪的状态)。\n1.前刺回马枪：在地面时释放，分为2段：前刺枪——[前摇0.35秒，后摇0.1秒](可移动释放，可被道具键取消前后摇（取消前摇会直接进入CD）)，对身前约450范围内的敌方造成{30}伤害，命中非虚弱敌方时获得30%加速，持续3秒；回马枪——[前摇0.35秒，后摇0.35秒](可移动释放，不可空中释放，可被道具键取消前后摇（取消前摇会直接进入CD）)，对身后约450范围内的敌方造成{30}伤害，击退对方约100距离，若命中[被前刺枪命中过的非虚弱目标](指被同一轮前刺回马枪的第一段命中过的目标，包括初次命中时处于虚弱状态的目标)，则额外造成{110}伤害和[0.5秒强控制状态](该状态实际效果与眩晕类似，但角色头顶并未出现“眩晕”图标；动作与击退类似，但击退距离为0，且会替代回马枪原本的击退效果)。\n2.跳跃劈枪：在空中时释放，[前摇0.1秒，后摇0.45秒](可移动释放，可被移动、跳跃、道具键取消前后摇（取消前摇会直接进入CD）)，对身前[约500范围内](500指横向判定范围。该攻击的纵向判定范围远大于常规爪刀)的敌方造成{55}伤害，命中非虚弱敌方时获得30%加速，持续3秒。',
      },
    ],
    counters: [
      {
      ...
      },
      {
        id: '米可',
        description:
          '米可极高的伤害反而更容易触发如玉被动技能的“坚毅”效果，并且如玉的前刺回马枪同时命中时能造成高达170点的基础伤害，一定程度上能反制米可的减伤。',
        isMinor: true,
      },
    ],
    counteredBy: [
      {
        id: '侦探杰瑞',
        description: '侦探杰瑞推奶酪速度快，自保和破局能力强，烟雾弹还能封锁技能。',
        isMinor: false,
      },
      {
        id: '牛仔杰瑞',
        description: '牛仔杰瑞的两个技能都不会直接触发如玉主动技能的“花枪反击”。',
        isMinor: false,
      },
      {
        id: '莱恩',
        description:
          '如玉的技能无法破除莱恩的方块，变身为“线条猫”后甚至无法攻击，但莱恩用三角或圆命中开启主动技能的如玉仍可触发其花枪反击。',
        isMinor: false,
      },
      {
        id: '天使泰菲',
    ...
    countersKnowledgeCards: [
      {
        id: '缴械',
        description: '如玉完全免疫缴械效果，注意不要在对抗如玉时携带缴械。',
        isMinor: false,
      },
      {
        id: '护佑',
        description: '如玉的掷花枪技能能破除护佑。',
        isMinor: true,
      },
    ],
  },
