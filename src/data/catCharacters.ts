    ...
    skills: [
      {
        ...
        detailedDescription:
          '随着音乐舞动，持续40秒。舞动开始时恢复50Hp。舞动时[免疫部分眩晕效果](包括控制道具和部分老鼠技能，但不免疫大部分NPC的控制)，爪刀变为以苏蕊为中心、范围更大的[舞动亮相](CD为6秒（未命中）和12秒（命中），伤害为70点)。移动和跳跃不会中断此技能。舞动时每隔13秒出现爱心提示，此时点击技能按钮将恢复30Hp、提升10%移速和15点攻击力。舞动时接触不在老鼠夹上的[虚弱老鼠](包括触发知识卡“铁血”的老鼠)将使其自主跟随苏蕊30秒，期间遇到火箭会立刻绑上。在此状态下，老鼠无法主动使用技能、移动等操作。若老鼠与苏蕊距离较远（如钻管道后），则老鼠会解除跟随状态。',
        canMoveWhileUsing: true,
        canUseInAir: true,
        cancelableSkill: '不可取消',
        cancelableAftercast: '无后摇',
