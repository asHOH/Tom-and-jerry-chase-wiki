  ...
  杰瑞: {
    ...
    skills: [
      ...
      },
      {
        ...
        type: 'weapon2',
        description:
          '吹响鸟哨召唤金丝雀。金丝雀会来回盘旋，持续向下方投掷{小鞭炮(鸟哨)(实体)}（命中敌方时爆炸，造成伤害和眩晕，被命中的敌方短暂免疫该道具效果；掉落一定时间后也会自行爆炸）。金丝雀轰炸一段时间后自行飞离。同一房间内最多只能有1只由鸟哨召唤的金丝雀。',
        detailedDescription:
          '在1秒前摇后吹响鸟哨，召唤金丝雀[飞到吹响鸟哨的位置](金丝雀会由地图左上角飞到目标位置，因此距离地图左上角越近，到达速度越快)，技能后摇1秒。金丝雀会[在一定范围内来回盘旋](金丝雀可穿墙)，每隔一段时间会向下方垂直投掷一枚{小鞭炮(鸟哨)(实体)}（命中敌方时爆炸，对目标造成55伤害和2秒眩晕，且可触发[投掷效果](指的是以投掷命中为条件的效果，包括知识卡-缴械/精准投射/投手/追风，特技-干扰投掷/勇气投掷)，但被命中的敌方[在眩晕期间及眩晕结束后1秒内](若眩晕被敌方护盾、霸体、无敌等效果免疫，则不会触发后续免疫效果)免疫该道具效果；掉落一定时间后也会自行爆炸，不造成伤害和眩晕）。金丝雀轰炸一段时间后自行飞离。[同一房间内最多只能有1只由鸟哨召唤的金丝雀](以吹响鸟哨的位置判定金丝雀所属房间，即使之后金丝雀穿越墙壁飞入其他房间也不会改变所属位置。金丝雀在飞到与飞离目标位置的过程中仍然算作存在于该房间内。与金丝雀NPC分开计数)。',
