    ...
    skills: [
      {
        ...
        type: 'active',
        description:
          '蓄力弹射，碰撞到其他目标时自身被反弹并对目标造成效果。弹射时点击技能将下坠并结束弹射。',
        detailedDescription:
          '长按技能键，在0.5秒前摇后开始蓄力，[拖拽并松开技能键进行弹射](弹射方向由技能拖拽的方向决定，初速度锁定为1675)，弹射时间随蓄力时间线性增加，最短1秒，最长4.7秒（[需要蓄力条满](满后仍能继续蓄力，但不增加弹射时间)，即蓄力2.5秒以上）。弹射过程中碰撞体积增大，碰撞到[其他目标](包括：猫咪、墙缝、小黄鸭、森林牧场大鸭子、墙壁、平台、地面)时将被[反弹](方向遵守反射定律。反弹平面为竖直面时，速度不变；为水平面时，改变速度：首次反弹初速度最高为1225，最低为1050，再次反弹初速度锁定为1050。碰撞猫咪/墙缝/鸭子时的反弹平面始终为竖直面，与他们的碰撞体积重叠期间只会进行一次反弹)。弹射时技能同步进入读条，再次点击技能将[竖直向下坠落1.5秒](初速度锁定为2000；下坠期间只会碰撞平台或地面)，下坠期间[无法进行其他操作](可用道具键*打断)，碰撞平台或地面时进入0.3秒不可被打断的后摇。前摇期间取消技能不进入CD，蓄力期间取消则进入50%的CD。', //使用了梦回的数值介绍 https://www.bilibili.com/video/BV1by4y1f7md/?t=10
        canMoveWhileUsing: false,
        canUseInAir: true,
        cancelableAftercast: '无后摇',
        cancelableSkill: '不可被打断',
