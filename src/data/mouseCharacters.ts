    ...
    skills: [
      {
        ...
        detailedDescription:
          '莱恩扔出{蓝图(投射物)(衍生物)}，碰到敌方/平台/地面（不包括{方块}/{强化方块}）、或莱恩再次使用本技能时，[展开](蓝图的贴图需要一小段时间才能完全展开，该动画期间莱恩无法使用技能收回蓝图，但蓝图效果在未完全展开时就已经生效)一个大小1000×666的{蓝图(召唤物)(衍生物)}。接触蓝图(召唤物)的{火箭}变为{线条火箭}（燃烧速度减半），猫咪[隐身状态暂时失效](离开蓝图时恢复)且受到伤害时会短暂变为线条猫，变身结束后一段时间内{免疫线条猫效果}，并使手中老鼠掉落。再次使用技能或持续较长时间后收回蓝图，蓝图收起后才会进入CD。\n线条猫：只可使用移动、跳跃和爪刀，但受到的伤害、爪刀CD、受控时间、虚弱时间减半。',
        canMoveWhileUsing: true,
        canUseInAir: true,
        forecast: 0.6,
        aftercast: 0.5,
