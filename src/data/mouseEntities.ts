...
export const mouseEntitiesDefinitions = {
  ...
  蓝图: {
    ...
    gravity: false,
    description:
      '接触蓝图的{火箭}变为{线条火箭}（燃烧速度减半），猫咪[隐身状态暂时失效](离开蓝图时恢复)且受到伤害时会短暂变为线条猫（有内置CD），并使手中老鼠掉落。莱恩再次使用技能或持续较长时间后收回蓝图。\n线条猫：线条猫：只可使用移动、跳跃和爪刀，但受到的伤害和[受控时间](包括虚弱时间)减少50%，爪刀CD减少40%。' +
      figureDescription,
    detailedDescription:
      '大小1000×666。接触蓝图的{火箭}变为{线条火箭}（燃烧速度减半），猫咪[隐身状态暂时失效](离开蓝图时恢复)且受到伤害时会短暂变为线条猫（变身结束后一段时间内不会再次变为线条猫），并使手中老鼠掉落。莱恩再次使用技能或持续较长时间后收回蓝图。\n线条猫：持续6秒，变身时手持道具与老鼠自然掉落。' +
      figureDescription,
