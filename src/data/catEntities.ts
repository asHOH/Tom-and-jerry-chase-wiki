...
const catEntitiesDefinitions: Record<string, EntityDefinition> = {
  ...
  托普斯分身: {
    ...
    detailedDescription:
      '其[与托普斯相貌相同](装扮、昵称、Hp条显示的方式和托普斯完全一致)。分身爪刀伤害提高15，爪刀CD减少，免疫碎片和老鼠夹，且[继承托普斯的部分知识卡](大多数知识卡均可继承，其中一部分知识卡效果还能和托普斯叠加（如“捕鼠夹”的伤害和控制时间延长效果可以二次叠加）；但部分特殊知识卡则无法继承或继承后无效果（如“熊熊燃烧”等）)。分身[共享小地图视野](由于人工智能类角色具有能看到隐身角色的特性，因此分身也能在小地图上透视周围隐身的老鼠，但不会主动攻击)，但[受到的伤害增加20](包括受到部分环境伤害时)。分身存在期间托普斯获得额外技能，点击可指挥分身出击或跟随（CD：5秒）。托普斯再次使用主动技能可与分身换位（有CD，与技能等级有关，与技能本身的冷却无关）。托普斯[获得部分增益时](包括食物、药水效果，以及部分地图道具效果（如太空堡垒-科研舱药水等）)，分身也会获得。\n\n升级效果：分身的存在时间随等级提高而增加，Lv.3时[永久存在](只会因Hp归零而消失)。Lv.2换位时回复Hp，且移速和交互速度短暂提高。Lv.3如果分身在本体附近，本体受到的[眩晕控制](包括大部分眩晕，另外还能免疫尼宝的鱼钩；不包括其它控制，如斯派克抓取、捕鼠夹、虚弱、仙女鼠8星等)和受到的一半伤害会转移给分身。',
    create: '由托普斯-双重猫格召唤。',
  },
  泡泡: {
  ...
  皇家火炮: {
    ...
    detailedDescription:
      '侍卫汤姆可以拖动技能键远程操纵火炮发射{炮弹}，命中时对老鼠造成0.56秒眩晕和[50伤害](不受攻击增伤加成)、移除其[部分增益](隐身、兴奋、远视；天宫图香炉的远视；除了尼宝三级翻滚和魔术师三级卡牌以外的技能与被动隐身)；同时侍卫汤姆加速49%并获得两层护盾，效果持续2.96秒。火炮可射击3/3/7次，每次射击之间有最短间隔。Lv.3时，炮弹对命中的老鼠额外施加减速、禁用其技能和道具键3.5秒。',
    create: '由侍卫汤姆-皇家火炮召唤。',
  },
  炮弹: {
    entitytype: '投射物类' as const,
  ...
  追求者: {
    ...
    detailedDescription:
      '奔跑时对碰触的老鼠造成伤害。Lv.3追求者额外造成[25](受其他攻击增伤影响)的爆炸伤害和控制。当{百科全书}在凯特附近闭合时，追求者会快速冲向书籍将其捡起并送还凯特。',
    create: '通过凯特-追求者出击召唤。',
  },
  百科全书: {
    entitytype: '投射物类' as const,
    ...
    detailedDescription:
      '书籍对直接命中的老鼠造成[25伤害](受攻击增伤影响)；落地后书籍会打开，书籍打开5/5/10秒后会自动闭合，也可以再次施放技能让书籍闭合，闭合后存在10秒。书籍闭合时[造成伤害](受其他攻击增伤影响)并将老鼠夹住，随后技能进入CD。书籍闭合后存在一定时间，凯特可以通过交互键捡起书籍返还5秒冷却并将被夹住的老鼠直接抓在手中。当书籍在凯特附近闭合时，{追求者}会快速冲向书籍将其捡起并送还凯特。\n书籍与夹子有相似和不同之处（可以触发{捕鼠夹}，但无法触发{夹不住我}、{狡诈}）。被投掷的书籍也无法触发{乾坤一掷}。\n\n升级效果：Lv.3书籍对直接命中的老鼠叠加3层破绽（详见{骄傲的学霸}）。',
  ...
  列车: {
    ...
    detailedDescription:
      '进行1.8秒的冲撞，沿{轨道}首端冲撞至末端，对被撞击的敌人施加50伤害，并造成眩晕和击退效果。冲撞完成后重新生成下一段列车，重复数次。被冲撞过的老鼠短时间内获得冲撞免疫效果（免疫列车冲撞带来的伤害和控制，但被撞击仍会造成小幅度位移）。\n\n升级效果：Lv.2列车经过恶魔汤姆会使恶魔汤姆获得一层护盾。撞击老鼠会使恶魔汤姆移速提高15%，[爪刀CD固定降低3.5秒](空刀CD由命中时的CD乘角色内置的空刀返还CD比例得到。',
  ...
  旋转的花枪: {
    ...
    create: '由如玉-掷花枪召唤并投掷。',
    specialImageUrl: '/images/catSkills/如玉2-掷花枪.png', //ToDo: add this entity's image
  },
};
