    ...
    skills: [
      {
        ...
        detailedDescription:
          '向前冲刺，冲刺持续1.2秒，期间加速并免疫控制效果，且撞飞碰到的道具（包括已放入{老鼠洞}的{奶酪}），并对撞到老鼠造成{1*,可致伤}伤害和0.5秒{眩晕}。如果命中老鼠，则自身增加{20}点攻击力，持续10秒。冲刺过程中持有霸体且可通过方向键多次改变冲刺方向，也可以使用爪刀、道具和技能。',
        aftercast: 0.2,
        canMoveWhileUsing: true,
        canUseInAir: true,
    ...
    skills: [
      {
        ...
        detailedDescription:
          '常规：向前方连续挥爪3次，前摇分别为0.3、0.4、0.6秒，每次命中敌方均造成[一定伤害](与技能等级有关)，范围均为300。（无法在空中释放；技能期间无法移动、跳跃、交互、使用道具或其它技能）\n疾冲状态下或{武器技能}飞行中：[以1560的速度](在武器技能飞行期间释放时，自身被牵引速度不变)向前穿刺0.45秒，命中敌方造成{50*,不可致伤}伤害和1.8秒{眩晕}，穿刺期间免疫控制。（可以在空中释放；技能期间无法转向、跳跃、交互、使用道具或其它技能；技能判定范围较大，可以隔门造成伤害）',
        canMoveWhileUsing: true,
        canUseInAir: true,
        cancelableAftercast: '无后摇',
        canHitInPipe: true,
