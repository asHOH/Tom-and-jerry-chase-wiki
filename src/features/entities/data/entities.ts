...
export const entityDefinitions = {
  ...
  金币符: {
    ...
    detailedDescription:
      '命中敌方造成50伤害，并扣除15%(至少为500)局内金币。被投掷时无前摇，可穿墙且被投掷时的初速度较高，可触发投掷类知识卡和特技：{缴械}、{投手}、{追风}、{精准投射}、{干扰投掷}、{勇气投掷}。',
  ...
  '乾坤袋-投射物': {
    ...
    detailedDescription:
      '未落地时可命中猫咪使其移速降低40%持续5秒，或命中墙缝造成[4](基础伤害3+霜月墙缝增伤1，同时也会受到其它来源的墙缝增伤影响)伤害，可触发投掷类知识卡和特技：{缴械}、{投手}、{追风}、{精准投射}、{干扰投掷}、{勇气投掷}；命中敌方角色/墙缝/平台/地面后生成{乾坤袋}，若因极端情况导致乾坤袋-投射物飞行时间过长，则在飞行数秒后会立刻生成乾坤袋。霜月再次使用技能时，乾坤袋变回投射物形态并飞回。',
  ...
  巨大泰菲球: {
    ...
    detailedDescription:
      '变身时立即恢复150Hp但掉落手持道具。变身期间移速提高并获得额外技能，可以进行开关门、钻管道等交互，但无法使用道具或是进行救援、推奶酪等交互。变身期间[免疫部分负面状态](受到伤害及眩晕等状态时会播放“炸毛”动画，该动画可被绝大多数操作取消，不影响行动)，并且{减伤}30%。变身持续20秒，时间耗尽或Hp归零时解除该状态。进入和解除该状态时有特殊动画（因Hp归零而解除该状态时除外）。',
    ...
    skills: [
      {
        ...
        detailedDescription:
          '朝圣者泰菲使用额头向前撞击，撞击范围为300单位距离，命中敌方时恢复自身10/10/25Hp并对敌方造成[35](基础伤害30+角色基础攻击增伤5，同时享受其它来源的攻击加成)伤害和持续2.5秒10％的减速效果，敌方处于撞击造成的减速效果期间被连续撞击三次会进入眩晕状态，持续1.5秒。撞击可对墙缝造成[3](基础伤害2.5+角色基础破坏0.5，同时享受其它来源的破坏加成)伤害。',
        canMoveWhileUsing: true,
        canUseInAir: true,
        forecast: 0.2,
        aftercast: 0.6,
  ...
  木塞子: {
    ...
    detailedDescription:
      '被发射后以1500/1500/1800(单位长度/秒)的速度在房间内反弹20/20/30秒，命中敌方造成[40](基础伤害35+角色基础攻击增伤5，同时享受其它来源的攻击加成)伤害并附加2秒的30%减速，命中头部则伤害改为[105](基础伤害100+角色基础攻击增伤5，同时享受其它来源的攻击加成)、同时为朝圣者泰菲额外补充1发武器技能存储次数(该效果不影响武器技能现有冷却，且内置10秒CD)、但目标2秒内免疫其它木塞子效果。木塞子从下方或侧方命中平台时也会被反弹。2级及以上时，木塞子命中[敌方猫咪](对敌方老鼠无该效果)为其附加9Hp/秒的持续伤害状态，持续3秒。3级时，木塞子可命中墙缝造成[3.5](基础伤害3+角色基础破坏0.5，同时享受其它来源的破坏加成)伤害。',
