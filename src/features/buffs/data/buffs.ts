...
const buffDefinitions: Record<string, BuffDefinition> = {
  ...
  免控: {
    ...
    aliases: ['弱霸体', '免疫控制'],
    description:
      '免疫{眩晕}等强控制效果，但通常$不$text-bond#免疫{伤害}、{虚弱}，以及{滑行}、{反向}、{失明}等效果。[不同免控效果免疫的范围不同](推测这是因为免控实际上由"白名单"实现，在这个名单上的状态均会被免疫，而不同免控的白名单差异很大)。该状态不会解除获得该状态前已受到的控制效果。持有该效果时，周身可能会出现恢复或减伤的特效，但仅仅只是借用特效，实际效果无关。',
    detailedDescription:
      '免疫{眩晕}等强控制效果，但通常$不$text-bond#免疫{伤害}、{虚弱}，以及{滑行}、{反向}、{失明}等效果。[不同免控效果免疫的范围不同](推测这是因为免控实际上由"白名单"实现，在这个名单上的状态均会被免疫，而不同免控的白名单差异很大)。该状态不会解除获得该状态前已受到的控制效果。持有该效果时，周身可能会出现恢复或减伤的特效，但仅仅只是借用特效，实际效果无关。' +
      shieldDifference,
    stack: commonStack,
  ...
  霸体: {
    ...
    aliases: ['强霸体'],
    description:
      '免疫{伤害}、{虚弱}、{眩晕}、{滑行}、{反向}、{失明}等负面效果，但也会免疫{隐身}。[不同霸体效果免疫的范围不同](推测这是因为霸体的免疫实际上由"白名单"实现，在这个名单上的状态均会被免疫，而不同霸体的白名单差异很大)。该状态不会解除获得该状态前已受到的控制效果。持有该效果时，周身可能会出现恢复或减伤的特效，但仅仅只是借用特效，实际效果无关。',
    detailedDescription:
      '免疫{伤害}、{虚弱}、{眩晕}、{滑行}、{反向}、{失明}等负面效果，但也会免疫{隐身}。[不同霸体效果免疫的范围不同](推测这是因为霸体的免疫实际上由"白名单"实现，在这个名单上的状态均会被免疫，而不同霸体的白名单差异很大)。该状态不会解除获得该状态前已受到的控制效果。持有该效果时，周身可能会出现恢复或减伤的特效，但仅仅只是借用特效，实际效果无关。' +
      shieldDifference,
    stack: commonStack,
  ...
  绝对霸体: {
    ...
    duration: '不固定',
    description:
      '免疫包括{伤害}、{虚弱}、{眩晕}、{反向}、{失明}在内的几乎全部负面状态/效果，但同时也会免疫{隐身}等部分正面状态/效果。该状态不会解除获得该状态前已受到的控制效果。',
    detailedDescription:
      '免疫包括{伤害}、{虚弱}、{眩晕}、{反向}、{失明}在内的几乎全部负面状态/效果，但同时也会免疫{隐身}等部分正面状态/效果。该状态不会解除获得该状态前已受到的控制效果。' +
      shieldDifference,
    stack: commonStack,
  ...
  变身: {
    ...
    detailedDescription:
      '变身为指定类型的衍生物。变身分为两种：\n1.通过对自身施加变身状态进行变身。这类变身效果持续期间受到的伤害和效果均会实时反映到角色本体上，此类包括：{变身饮料}、{头盔}、{大星星}、{小星星}、{线条猫}等。\n2.隐藏自身模型，临时切换自身键位，改为操作另一种指定类型的衍生物。这类变身效果的变身对象受到的伤害和效果不会对角色本体造成影响，但部分能对{模型消失}目标造成效果的技能仍能直接对本体造成影响：{机器鼠(衍生物)}、{战车}、{乾坤袋(衍生物)}、盔甲人等。\n\n变身效果的优先级很高，通常不会因{霸体}、{虚弱}等效果而免疫。',
  ...
  鼠虚弱: {
    ...
    failure: '得到队友治疗后',
    description:
      '禁用技能和道具，获得高额减速及跳跃高度降低，且可被{抓起}，持续10秒。其它老鼠可通过交互键治疗虚弱的老鼠，治疗共需2.4秒（多名老鼠可合作加速治疗），治疗期间双方无法移动，成功治疗时获得少量经验（不会解除{受伤}）。',
    stack: '角色虚弱期间不会再次受到虚弱效果，也不会重置虚弱时间。',
    source: [
