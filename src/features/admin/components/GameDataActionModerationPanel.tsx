          ...
          <div className='space-y-3'>
            {pendingActions.map((submission) => (
              <GameDataActionRow
                ...
                isExpanded={expandedActionIds.has(submission.action_id)}
                diffView={diffView}
                showAllDiffContext={showAllDiffContext}
                copyText={copyText}
                moderateAction={moderateAction}
                onThank={setThankTarget}
                onToggleSelected={() => toggleSelectedAction(submission.action_id)}
                onToggleExpanded={() => toggleExpanded(submission.action_id)}
              />
            ))}
          </div>
        )}
