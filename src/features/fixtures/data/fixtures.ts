...
const FixtureDefinitions: Record<string, FixtureDefinition> = {
  ...
  墙缝: {
    ...
    detailedDescription:
      '墙缝是鼠方逃离点。墙缝会显示在小地图上。\n在{经典奶酪赛}等模式中，[奶酪期结束时](经典奶酪赛中的奶酪期与墙缝期之间增加了10秒间隔，因此会出现墙缝出现并隐形的情况。部分休闲模式中没有这个间隔，墙缝会正常出现)会立即在[随机位置](根据地图不同有数个预设位置，通常每个房间最多1个预设位置)生成[隐形的墙缝](此时墙缝处于隐形状态，正常情况下无法被攻击，但在极少数情况下可被部分效果攻击到)并显示在小地图上，墙缝期开始时墙缝现形并可进行破坏。墙缝初始拥有100Hp，Hp归零时被破开，随后老鼠可与其交互逃脱，累计逃脱2只老鼠即可胜利。\n在{疯狂奶酪赛}中，墙缝被鼠方一击即溃。',
  },
  桶: {
    type: ['物件类'],
    source: '通用组件',
  ...
  吊灯: {
    ...
    collsion: ['道具', '角色'],
    description:
      '初始情况下提供落脚点和灯光。角色可通过交互将其打落，对碰到的其他角色造成伤害和眩晕，坠地后生成数个{碎片}。',
    detailedDescription:
      '吊灯初始情况下可承载其上方的物体。角色与吊灯交互后可将其打落，坠落期间会对除击落者外的其他接触的玩家造成伤害和眩晕（不分敌我），坠落至地面后生成数个{碎片}。\n未被打落的吊灯与{平台}类似，但部分[需选中平台的技能](例如罗宾汉登山飞镐，兔子大表哥指卧撑等)无法选中吊灯。\n吊灯与{开关}存在联动关系：当开关关闭时，未被打落的吊灯可以充当光源。',
  },
  开关: {
    type: ['组件类', '可交互'],
    source: '地图组件',
