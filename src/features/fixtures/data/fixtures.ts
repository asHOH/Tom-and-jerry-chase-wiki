...
const FixtureDefinitions: Record<string, FixtureDefinition> = {
  ...
  折叠椅: {
    ...
    supportedMaps: ['夏日游轮I', '夏日游轮II', '夏日游轮III'],
    description:
      '折叠椅可被当作{弹性平台}，但当被角色踩踏第3次时将折叠，折叠期间平台短暂[消失](不再阻挡角色和道具，但外观贴图无变化)，且处于折叠椅位置的所有角色受到短暂{眩晕}（不掉落道具）；随后折叠椅重新展开并重置计数。',
    detailedDescription:
      '折叠椅可被当作{弹性平台}，但[每当被角色踩踏时将累计计数](会播放“弹性”动画以作提示；动画期间有角色踩踏时不会计数)，计数累积到3时立即折叠自身，折叠期间平台短暂[消失](不再阻挡角色和道具，但外观贴图无变化)，且处于折叠椅位置的所有角色受到短暂{眩晕}（不掉落道具）；随后折叠椅重新展开并重置计数。',
  },
  游泳圈组: {
    type: ['平台类', '组件类'],
    source: '地图组件',
