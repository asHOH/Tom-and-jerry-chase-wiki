...
export const catKnowledgeCards: Record<string, Card> = {
  ...
  乘胜追击: {
    ...
    detailedDescription:
      '爪刀命中非虚弱老鼠或使老鼠进入虚弱时，提升2.5%移速和2%爪刀频率，可**叠加**（每层加成间乘算）。猫咪进入虚弱状态移除当前一半层数。（与{律动时间}+{蓄势一击}有特殊互动关系，详见二者界面）',
    levels: [
  ...
  蓄势一击: {
    ...
    detailedDescription:
      '每一次的潜伏，都是为了今后的爆发。每隔**一段时间**，获得蓄势一击状态，下次爪刀附带[额外伤害](视为两段伤害，但若第一段伤害被护盾抵挡则不会造成第二段伤害；伤害类型与爪刀不同，不会触发霜月Lv.3被动等效果，但有一个例外——当苏蕊同时携带蓄势+乘胜，且开启律动时间时，舞动亮相+蓄势一击会额外叠加一层乘胜追击效果)，[基础伤害为50](会受其他来源的攻击增伤等效果影响，因此在绝大多数情况下伤害与爪刀相同)，携带{长爪}时改为[0](且改为不受攻击增伤等效果影响)。被老鼠眩晕会移除该状态。虚弱起身或角色升级后立刻获得该状态。',
    levels: [
  ...
  加大火力: {
    ...
    detailedDescription:
      '点燃火箭吧！火箭燃烧速度变得**更快**！在绑有老鼠的火箭半径1100范围内击倒老鼠，将额外减少火箭5秒倒计时。(CD：20秒，[火箭上的不同老鼠独立](减燃烧效果生效目标为火箭上的老鼠，生效后20秒内不会对该老鼠重复生效；不同老鼠的CD互相独立))',
    levels: [
  ...
  巡逻戒备: {
    ...
    detailedDescription: '四处巡逻的猫咪，**降低**附近1100范围内老鼠的推速。',
    levels: [
