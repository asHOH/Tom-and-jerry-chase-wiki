...
function TraitItemEditor({
  item,
  updateItem,
  onRemove,
}: {
  item: SingleItemOrGroup;
  updateItem: EditableUpdate<SingleItemOrGroup>;
  onRemove: () => void;
}) {
  return (
    <div className='border-border bg-surface-raised grid gap-2 rounded-lg border p-2 sm:grid-cols-[minmax(0,1fr)_10rem_7rem_auto]'>
      ...
      <FormSelect
        ...
        onChange={(event) => {
          updateItem((draft) => {
            draft.type = event.target.value as SingleItemOrGroup['type'];
            if (draft.type === 'itemGroup') delete (draft as { factionId?: FactionId }).factionId;
          });
        }}
      >
      ...
      <FormSelect
        ...
        onChange={(event) => {
          updateItem((draft) => {
            if (draft.type === 'itemGroup') return;
            if (event.target.value === 'cat' || event.target.value === 'mouse') {
              draft.factionId = event.target.value;
            } else {
              delete draft.factionId;
            }
          });
        }}
      >
...
function TraitGroupEditor({
  group,
  updateGroup,
}: {
  group: TraitGroup;
  updateGroup: EditableUpdate<TraitGroup>;
}) {
  const removeItem = (groupIndex: number, alternativeIndex: number) => {
    updateGroup((draft) => {
      const groupItem = draft[groupIndex];
      ...
      if (groupItem.length === 0) draft.splice(groupIndex, 1);
      else if (groupItem.length === 1) draft[groupIndex] = groupItem[0]!;
    });
  };
  ...
  const addAlternative = (groupIndex: number) => {
    updateGroup((draft) => {
      ...
      else draft[groupIndex] = [groupItem, createEmptyItem()];
    });
  };
  ...
  return (
    <div className='space-y-3'>
              ...
              {alternatives.map((item, alternativeIndex) => (
                <TraitItemEditor
                  ...
                  updateItem={(mutate) =>
                    updateGroup((draft) => {
                      const groupItem = draft[groupIndex];
                      const current = Array.isArray(groupItem)
                        ? groupItem[alternativeIndex]
                        : groupItem;
      ...
      <Button
        ...
        onClick={() =>
          updateGroup((draft) => draft.push(createEmptyItem() as MutableTraitGroupItem))
        }
      >
        添加关联组
      </Button>
...
function TraitCard({
  ...
  updateTraits: EditableUpdate<Record<string, Trait>>;
}) {
  ...
  const updateTrait: EditableUpdate<Trait> = (mutate) => {
    updateTraits((draft) => {
      const current = draft[traitId];
      if (!current) throw new Error(`Cannot edit trait ${traitId} because it is not loaded.`);
      mutate(current);
    });
  };
  ...
  const commitRename = () => {
    ...
    updateTraits((draft) => {
      const current = draft[traitId];
      if (!current) throw new Error(`Cannot rename trait ${traitId} because it is not loaded.`);
      draft[nextId] = current;
      delete draft[traitId];
    });
        ...
        <div className='border-border space-y-5 border-t p-4'>
          <div className='grid gap-4 md:grid-cols-[minmax(0,1fr)_12rem_auto]'>
            ...
            <label className='space-y-1 text-sm font-medium'>
              ...
              <FormSelect
                ...
                onChange={(event) => {
                  ...
                  updateTrait((draft) => {
                    if (value === 'cat' || value === 'mouse') draft.excludeFactionId = value;
                    else delete draft.excludeFactionId;
                  });
                }}
              >
            ...
            <Button
              ...
              onClick={() => {
                if (window.confirm(`确认删除特性 ${traitId} 吗?`)) {
                  updateTraits((draft) => delete draft[traitId]);
                }
              }}
            >
              删除特性
          ...
          <section className='space-y-2'>
            ...
            <TraitGroupEditor
              group={trait.group}
              updateGroup={(mutate) => updateTrait((draft) => mutate(draft.group))}
            />
          </section>
            ...
            <div className='flex items-center justify-between gap-3'>
              ...
              <Button
                ...
                onClick={() => {
                  updateTrait((draft) => {
                    draft.spacialCase ??= [];
                    draft.spacialCase.push({ description: '', group: [] });
                  });
                }}
              >
                添加特殊情况
              ...
              >
                <div className='flex items-center justify-between gap-3'>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      updateTrait((draft) => {
                        draft.spacialCase?.splice(specialCaseIndex, 1);
                        if (draft.spacialCase?.length === 0) delete draft.spacialCase;
                      });
                    }}
                  >
                    删除
                ...
                <FormTextarea
                  ...
                  onChange={(event) => {
                    updateTrait((draft) => {
                      const current = draft.spacialCase?.[specialCaseIndex];
                ...
                <TraitGroupEditor
                  ...
                  updateGroup={(mutate) =>
                    updateTrait((draft) => {
                      const current = draft.spacialCase?.[specialCaseIndex];
                      if (current) mutate(current.group);
                    })
                  }
                />
              </div>
...
export default function TraitEditor({ traitsSnapshot }: { traitsSnapshot: Record<string, Trait> }) {
  const [traits, updateTraits] = useEditableDomain(
    'traits',
    traitsSnapshot
  ) as unknown as readonly [Record<string, Trait>, EditableUpdate<Record<string, Trait>>];
  const [query, setQuery] = useState('');

  const filteredEntries = useMemo(() => {
    const normalizedQuery = query.trim().toLocaleLowerCase('zh-CN');
    return Object.entries(traits).filter(([id, trait]) => {
  ...
  return (
    <div className='mx-auto max-w-5xl space-y-4 px-2 py-4 sm:px-4'>
      ...
      <p className='text-muted-foreground text-sm'>
        共 {Object.keys(traits).length} 条特性，当前显示 {filteredEntries.length}{' '}
