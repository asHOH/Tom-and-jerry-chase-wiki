            ...
            <div className='mt-6 space-y-3'>
              ...
              <div className='grid grid-cols-2 gap-3'>
                  ...
                  <p className='text-sm text-gray-700 py-1'>
                    <Tooltip
                      content={getTooltipContent(
                        'Hp上限',
                        character.faction.id as 'cat' | 'mouse',
                        isDetailedView
                      )}
                    >
                      Hp上限
                ...
                {character.faction.id === 'cat' && character.clawKnifeRange && (
                  <p className='text-sm text-gray-700 py-1'>
                    <Tooltip content={getTooltipContent('爪刀范围', 'cat', isDetailedView)}>
                      爪刀范围
                    </Tooltip>
                ...
                {character.faction.id === 'cat' &&
                  character.clawKnifeCdHit &&
                  character.clawKnifeCdUnhit && (
                    <p className='text-sm text-gray-700 py-1'>
                      <Tooltip content={getTooltipContent('爪刀CD', 'cat', isDetailedView)}>
                        爪刀CD
                      </Tooltip>
                      : {character.clawKnifeCdUnhit} / {character.clawKnifeCdHit} 秒
                    </p>
                  )}
                ...
                {character.faction.id === 'mouse' &&
                  ...
                  character.attackBoost !== 0 && (
                    <p className='text-sm text-gray-700 py-1'>
                      <Tooltip content={getTooltipContent('攻击增伤', 'mouse', isDetailedView)}>
                        攻击增伤
                      </Tooltip>
                      : {character.attackBoost}
                    </p>
                  )}
                {character.faction.id === 'mouse' && character.cheesePushSpeed && (
                  <p className='text-sm text-gray-700 py-1'>
                    <Tooltip content={getTooltipContent('推速', 'mouse', isDetailedView)}>
                      推速
                    </Tooltip>
                    : {character.cheesePushSpeed} %/秒
                  </p>
                )}
                {character.faction.id === 'mouse' && character.wallCrackDamageBoost && (
                  <p className='text-sm text-gray-700 py-1'>
                    <Tooltip content={getTooltipContent('墙缝增伤', 'mouse', isDetailedView)}>
                      墙缝增伤
                    </Tooltip>
              ...
              {character.faction.id === 'cat' &&
                ...
                character.attackBoost !== 0 && (
                  <p className='text-amber-600 py-1'>
                    <Tooltip content={getTooltipContent('攻击增伤', 'cat', isDetailedView)}>
                      攻击增伤
                    </Tooltip>
                    : {character.attackBoost}
                  </p>
              ...
              {character.faction.id === 'cat' &&
                ...
                character.catPositioningTags.length > 0 && (
                  <div className='mt-6 pt-4 border-t border-gray-200'>
                    ...
                    <div className='space-y-3'>
                      {character.catPositioningTags.map((tag, index) => (
                        <div
                          key={index}
                          className={`rounded-lg p-3 ${getPositioningTagContainerColor(tag.tagName, tag.isMinor, 'cat')}`}
                        >
              ...
              {character.faction.id === 'mouse' &&
                ...
                character.mousePositioningTags.length > 0 && (
                  <div className='mt-6 pt-4 border-t border-gray-200'>
                    <h3 className='text-lg font-semibold text-gray-800 mb-3'>定位</h3>
                    <div className='space-y-3'>
                      {character.mousePositioningTags.map((tag, index) => (
                        <div
                          key={index}
                          className={`rounded-lg p-3 ${getPositioningTagContainerColor(tag.tagName, tag.isMinor, 'mouse')}`}
                        >
                          <div className='flex items-center gap-2 mb-2'>
                            <Tag
                              colorStyles={getPositioningTagColors(
                      ...
                      <div className='flex-shrink-0 mr-6'>
                        ...
                        {skill.videoUrl && (
                          <div className='mt-2'>
                            <button
                              onClick={() =>
                                window.open(skill.videoUrl, '_blank', 'noopener,noreferrer')
                              }
                    ...
                    <div className='flex-1'>
                        ...
                        <div className='text-sm text-gray-500 mt-1 px-2'>
                          {(() => {
                            ...
                            if (skill.skillLevels.some((level) => level.cooldown)) {
                              const cooldowns = skill.skillLevels.map(
                      ...
                      {(() => {
                        const descriptionText =
                          ...
                          skill.detailedDescription.trim() !== ''
                            ? skill.detailedDescription
                            : skill.description;
                      ...
                      {skill.skillLevels.map((level) => (
                        ...
                        >
                          <p className='px-2 py-1'>
                            ...
                            level.detailedDescription.trim() !== ''
                              ? level.detailedDescription
                              : level.description}
                          </p>
                        </div>
