    ...
    >
      <div className='relative rounded-xl bg-white/95 py-3 pr-4 pl-10 shadow-lg ring-1 ring-gray-200 backdrop-blur-sm dark:bg-slate-800/95 dark:ring-slate-700'>
        ...
        <div className='flex items-center gap-2'>
          {/* Discard */}
          <Button
            variant={isConfirmingDiscard ? 'danger' : 'secondary'}
            size='sm'
          ...
          >
            <span className='hidden sm:inline'>
              {isDirty ? (isConfirmingDiscard ? '确认放弃？' : '放弃') : '退出'}
            </span>
          </Button>
          ...
          <Button
            ...
            leadingIcon={<EyeIcon size={16} strokeWidth={2} />}
          >
          ...
          >
            <span>{isPublishing ? '发布中...' : showMessageInput ? '确认发布' : '发布'}</span>
          </Button>
        </div>
      </div>
    </m.div>
