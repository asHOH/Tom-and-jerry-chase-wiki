...
export type ChangeType = 
  ...
  | 'other';    // Other changes that doesn't fit above categories
...
JSON 应是一个 AIChangeEntry 对象数组，具有以下确切的 TypeScript 结构：

${TYPE_DEFINITIONS}

**转换规则：**
1. 从常规提交格式（feat、fix、docs 等）中提取类型 type
2. 从括号中提取范围 scope
3. 将提交消息翻译成简洁的中文描述作为更改说明
4. 如果提交中有 "BREAKING CHANGE" 或类型中有 "!"，则标记为破坏性更改
5. **合并目的相似的连续提交**（注意，不是合并scope相同的提交。请勿合并scope相同但目的不同的提交）
6. **hashes 字段必须是包含该组中所有提交哈希的数组**
7. **重要：message 字段必须使用中文、结尾无需标点**
8. **重要：scope 字段必须按照映射示例生成。如果不在示例中，则根据原始 scope 的清晰程度，选择翻译或保留原英文；如果映射的结果为空（比如原始 scope 为 dev），则舍掉这条提交记录**
...
async function generateChangelogs(commits) {
  ...
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(commits.length / BATCH_SIZE)}...`
    );
    ...
    try {
      ...
      allAiChanges = allAiChanges.concat(aiChanges);
    } catch (error) {
      console.error('Error in batch:', formatError(error));
      throw error; // Let outer try/catch handle the skip
    }
  }

  return groupAIChangesByDate(allAiChanges, commits);
}
