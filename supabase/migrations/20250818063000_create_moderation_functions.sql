...
CREATE OR REPLACE FUNCTION public.approve_article_version(p_version_id uuid, p_reviewer_id uuid)
RETURNS void AS $$
BEGIN
    -- Check if the reviewer has the right permissions
    IF get_user_role(p_reviewer_id) NOT IN ('Reviewer', 'Coordinator') THEN
...
CREATE OR REPLACE FUNCTION public.reject_article_version(p_version_id uuid, p_reviewer_id uuid)
RETURNS void AS $$
BEGIN
    -- Check if the reviewer has the right permissions
    IF get_user_role(p_reviewer_id) NOT IN ('Reviewer', 'Coordinator') THEN
...
CREATE OR REPLACE FUNCTION public.revoke_article_version(p_version_id uuid, p_reviewer_id uuid)
RETURNS void AS $$
DECLARE
...
BEGIN
    ...
    IF get_user_role(p_requester_id) NOT IN ('Contributor', 'Reviewer', 'Coordinator') THEN
    ...
    WHERE av.status IN ('pending', 'rejected')
      AND (
        get_user_role(p_requester_id) IN ('Reviewer', 'Coordinator')
        OR av.editor_id = p_requester_id
      )
